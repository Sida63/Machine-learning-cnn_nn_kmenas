...
def InitCNN(num_channels, filter_size, num_filters_1, num_filters_2,
    ...
    v1=0 * np.random.randn(filter_size, filter_size,
                               num_channels,  num_filters_1)
    v2=0 * np.random.randn(filter_size, filter_size,
                               num_filters_1, num_filters_2)
    v3=0 * np.random.randn(num_filters_2 * 64, num_outputs)
    vb1=np.zeros((num_filters_1))
    vb2=np.zeros((num_filters_2))
    vb3=np.zeros((num_outputs))
    model = {
        'W1': W1,
        'W2': W2,
        'W3': W3,
...
'''
    plt.figure(2)
    plt.clf()
    for i in xrange(model['W1'].shape[3]):
        for j in xrange(model['W1'].shape[])
        plt.subplot(1, model['W1'].shape[3], i+1)
        plt.imshow(model['W1'][:,:,i].reshape(48, 48), cmap=plt.cm.gray)
    plt.draw()
    plt.pause(1)
    '''
