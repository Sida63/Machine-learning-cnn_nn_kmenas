...
def InitNN(num_inputs, num_hiddens, num_outputs):
    ...
    v1=0 * np.random.randn(num_inputs, num_hiddens[0])
    v2=0 * np.random.randn(num_hiddens[0], num_hiddens[1])
    v3=0 * np.random.randn(num_hiddens[1], num_outputs)
    vb1=np.zeros((num_hiddens[0]))
    vb2=np.zeros((num_hiddens[1]))
...
def main():
    ...
    plt.figure(2)
    plt.clf()
    plt.subplot(1, model['W1'].shape[1], 2)
    plt.imshow(model['W1'][:, 1].reshape(48, 48), cmap=plt.cm.gray)
    plt.draw()
    plt.pause(1)
