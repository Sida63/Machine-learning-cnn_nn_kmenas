...
def q2():
    ...
    #------------------- Add your code here ---------------------
    #------------------- Answers ---------------------
    p, mu, vary, logProbX = mogEM(inputs_train, K, iters, randConst, minVary)
    #print logProbX
    print p
    #print mu
    #print vary
...
def q4():
    ...
    for t in xrange(T):
        ...
        #------------------- Answers ---------------------
        p2, mu2, vary2, logProbX2 = mogEM(x_train_anger, K, iters, randConst, minVary)
    
        p3, mu3, vary3, logProbX3 = mogEM(x_train_happy, K, iters, randConst, minVary)
        logProb2tr = mogLogLikelihood(p2, mu2, vary2, x_train)
        logProb3tr = mogLogLikelihood(p3, mu3, vary3, x_train)

        logProb2v = mogLogLikelihood(p2, mu2, vary2, x_valid)
        logProb3v = mogLogLikelihood(p3, mu3, vary3, x_valid)
        
        logProb2t = mogLogLikelihood(p2, mu2, vary2, x_test)
        logProb3t = mogLogLikelihood(p3, mu3, vary3, x_test)

    # classify
        errorTrain[t] = np.mean((logProb2tr > logProb3tr) == y_train)
        errorValidation[t] = np.mean((logProb2v > logProb3v) == y_valid)
        errorTest[t] = np.mean((logProb2t > logProb3t) == y_test)
